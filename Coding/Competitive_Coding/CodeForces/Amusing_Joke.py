...
# One night, when everyone went to bed, someone took all the letters of our characters' names. Then he may have shuffled
# the letters and put them in one pile in front of the door.

# The next morning it was impossible to find the culprit who had made the disorder. But everybody wondered whether it is
# possible to restore the names of the host and his guests from the letters lying at the door? That is, we need to
...
# Input
# The input file consists of three lines: the first line contains the guest's name, the second line contains the name of
# the residence host and the third line contains letters in a pile that were found at the door in the morning.
...
#   NO
# Note
# In the first sample the letters written in the last line can be used to write the names and there won't be any
# extra letters left.
...
char_occur = {}
flag = 1
char_list = list(set(pile))
for i in char_list:
    char_occur[i] = pile.count(i)
for i in set(guest_name):
    if i in char_occur:
        char_occur[i] -= guest_name.count(i)
    else:
        flag = 0
        break
for i in set(res_host):
    if i in char_occur:
        char_occur[i] -= res_host.count(i)
    else:
        flag = 0
        break
f = 1
